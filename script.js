...
const obstacleTypes = [
    ...
    { type: 'comet', color: '#9ad7ff' },
    { type: 'satellite', color: '#cfd5ff' },
    { type: 'planet', color: '#f8e36f' },
    ...
    for (const obs of state.obstacles) {
        ...
        switch (obs.type.type) {
            ...
            case 'comet': {
                ...
                ctx.lineWidth = 4;
                ctx.beginPath();
                ctx.moveTo(-obs.radius * 2, 0);
                ctx.quadraticCurveTo(0, -obs.radius * 0.5, obs.radius, 0);
                ctx.stroke();
                ctx.fillStyle = '#ffffff';
                ctx.beginPath();
                ctx.arc(obs.radius, 0, obs.radius * 0.6, 0, Math.PI * 2);
                ctx.fill();
                break;
            }
            case 'satellite': {
                ctx.fillStyle = obs.type.color;
                ctx.fillRect(-obs.radius * 0.6, -obs.radius * 0.3, obs.radius * 1.2, obs.radius * 0.6);
                ctx.fillStyle = '#9aa6ff';
                ctx.fillRect(-obs.radius, -obs.radius * 0.2, obs.radius * 0.4, obs.radius * 0.4);
                ctx.fillRect(obs.radius * 0.6, -obs.radius * 0.2, obs.radius * 0.4, obs.radius * 0.4);
                break;
            }
            case 'planet': {
